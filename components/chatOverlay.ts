...
export default class ChatOverlayExtension {
  ...
  _arisChat?: St.BoxLayout;
  _floatingInputBar?: St.BoxLayout;
  BoxLayout?: St.ScrollView;
  _peekHeight = 62;
  messages: (UserMessage | AIMessage)[] = [];
  _xAlignConstraint?: Clutter.AlignConstraint;
  _yAlignConstraint?: Clutter.AlignConstraint;
  _arisScrollView?: St.ScrollView;
  _windowControl?: St.BoxLayout;
  _messsagesBox?: St.BoxLayout;
  _signalId?: number;
  _inputEnterEventId?: number;
  _globalFocusWindowId?: number;
  _globalWindowCreatedId?: number;
  callAris: (query: string) => void;
  _chatInputBar?: St.BoxLayout;
  _state: "floating" | "chat" = "floating";

  constructor(dir: Gio.File, callAris: (query: string) => void) {
    this.dir = dir;
    this.callAris = callAris;
  }

  createInputBar(mode: "floating" | "chat") {
    let bar: St.BoxLayout;
    if (mode === "floating") {
    ...
    const entry = new St.Entry({
      ...
      hint_text:
        mode === "floating" ? "Where should we start?" : "Ask anything",
      can_focus: true,
      reactive: true,
    });
  ...
  createIcon(name: string, size: number, styleClass?: string) {
    return new St.Icon({
      gicon: new Gio.FileIcon({
  ...
  addNewBubble(message: UserMessage | AIMessage) {
  ...
  submitQuery(text: Clutter.Text, callAris: (query: string) => void) {
    ...
    callAris(value);
    let userMessage: UserMessage = { content: value, role: "user" };
  ...
  collapse() {
    if (!this._floatingInputBar || this._isCollapsed) return;
  ...
  dockToBottom() {
    if (!this._floatingInputBar) return;
  ...
  connectWindowListeners() {
    this._globalFocusWindowId = global.display.connect(
      "notify::focus-window",
      () => {
    ...
    this._globalWindowCreatedId = global.display.connect(
      "window-created",
      () => {
        if (this._isCollapsed) return;
        this.updateOverlayPosition();
  ...
  createChatOverlay() {
    ...
    this._inputEnterEventId = this._floatingInputBar.connect(
      "enter-event",
      () => {
    ...
    this._signalId = Gio.DBus.session.signal_subscribe(
      ...
      (_conn, _sender, _path, _iface, _signal, params) => {
        const [message] = params.deep_unpack() as [AIMessage];
