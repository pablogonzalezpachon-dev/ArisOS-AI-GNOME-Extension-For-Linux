...
import TitleBarExtension from "./components/titleBar.js";
...
export default class ArisDockExtension extends Extension {
  _arisMenu?: St.BoxLayout;
  _textInput?: St.BoxLayout;
  _windowCreatedId?: number;
  _titlebars = new Map<Meta.Window, TitleBarExtension>();
  _arisOverlay?: St.BoxLayout;

  callAris(command: string) {
    Gio.DBus.session.call(
      ...
      (conn, res) => {
        try {
          const result = (conn as Gio.DBusConnection).call_finish(res);

          // 🔑 D-Bus always returns tuples
          const [response] = result?.deep_unpack() as [string];
          log(response);
        } catch (e) {
          logError(e as Error, "Failed to call Aris");
        }
      },
    );
  }
  ...
  enable() {
    ...
    (Main.panel as any)._leftBox.insert_child_at_index(this._arisMenu, 0);

    const chatOverlayExtension = new ChatOverlayExtension(
      this.dir,
      this.callAris,
    );
    ...
    (Main.panel.statusArea as any).activities?.container.hide();
